...
**`--multi-line`** renders the POSIX session hook as a readable multi-line script instead of the single physical line `render.sh` emits by default. The fold is there because an MDM in the delivery path can rewrite the command's line endings: a Kandji Custom Profile upload was observed turning every LF in a `SessionStart` command into CRLF, and `esac\r` isn't the `esac` keyword, so the bootstrap stopped parsing and the hook exited 2 on every session while the per-event hooks kept reporting (see [deploy-claude-profile.md](docs/deploy-claude-profile.md#3-verify)). A command with no line breaks has nothing to rewrite. Use `--multi-line` to read a rendered config by eye, not to deploy one. `scripts/download_endorctl.sh` is unaffected either way — it stays the multi-line source of truth, and only the rendered payload is folded.
...
scripts/
  ...
  render.sh               generate a tool's config (JSON, or Codex TOML)
