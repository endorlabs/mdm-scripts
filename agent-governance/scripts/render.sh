...
# JSON. The command is escaped with the same `js` escaper the JSON builders use;
# its output (\n \" \\) is equally valid inside a TOML basic string "...". One
# artifact carries all events: on a managed/requirements source Codex marks these
# hooks trusted-by-policy, so they run with no per-user approval.
codex_event() {  # $1=event  $2=js-escaped command body  $3=matcher or ""
  printf '[[hooks.%s]]\n' "$1"
