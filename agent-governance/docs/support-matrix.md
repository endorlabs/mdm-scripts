...
- **The runner needs `git` + `jq` on the endpoint** (rows 2, 3, 7). The profile path (row 6) and the Windows pre-generate path (rows 4, 8) need nothing beyond what ships with the OS. If you'd rather not add `git`/`jq` to Linux endpoints, deliver the pre-generated file via config management instead.
...
- **Generation is POSIX-only** — `render.sh` needs `sh` + `jq` (and `iconv` + `base64` for Windows targets). A Windows-only admin generates under WSL or Git Bash.
