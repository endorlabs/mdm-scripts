...
`~/.config/endor/env.sh` is no longer a static block — it is generated by `templates/envsh.sh` from the user-attribution credentials computed at install time in `templates/testing.sh` (which derives `${ENDOR_ATTR_USER}` etc. from `<console-user>@<machine>`).
