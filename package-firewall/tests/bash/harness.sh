...
# ─── Working-tree copy, for the end-to-end suites ─────────────────────────────
# The generators write to <generator dir>/out/<namespace>, with no override. The
# e2e suites need to generate with throwaway credentials — and to regenerate with
# rotated ones — so they copy the working tree into a sandbox and run the
# generator there. That keeps the checkout clean and keeps the branch additive:
# no product code is changed to accommodate the tests.
